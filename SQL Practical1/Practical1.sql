...
('Thermo Meter','Health Care',550,'A Thermo Meter version M23o'),
...
('Limcee','Health Care',120,'A Vitamin - C Tablet'),
('Wooden toys','Toys',500,'A wooden sticks to make Wooden home'),
('Paracetamol Tablets','Health Care',50,'Used to treat fever and mild to modrate pain')

SELECT * FROM tblProducts

--Write a Query to List all the products belonging to health category
SELECT * FROM tblProducts WHERE	Category = 'Health Care'
...
SELECT Category,COUNT(ProductId) AS 'Total Products' 
FROM tblProducts 
WHERE Price > 10000
GROUP BY Category 
